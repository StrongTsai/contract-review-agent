...
```bash
.venv/bin/python main.py samples/contract.txt          # 文件输入
...
详见 [`docs/pitfalls.md`](pitfalls.md)(独立维护,写代码遇坑主动追加)。已覆盖 9 条:文件名单复数、密钥泄露、`load_dotenv`、interrupt 需 checkpointer、`next` 非空≠有 interrupt、空暂停续跑传 `None`、state 字段声明、msgpack 白名单、驳回仍生成报告。

## 8. 后续开发计划

1. ~~RAG 知识库 + 工具调用(agent 化)~~ ✅ 已完成:review 节点内嵌 create_agent,自主查证。
2. OCR 接入(PaddleOCR / macOS Vision)→ 多模态数字员工。
3. supervisor 多智能体。
4. LangSmith 追踪的 token/成本分析。
